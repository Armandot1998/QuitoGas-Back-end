...
const updateSucursal = async (req, res) => {
    const id = req.params;
    const { nombre_sucursal, telefono_sucursal, direccion_sucursal, correo_sucursal } = req.body.sucursal;
    sucursal = await pool.query('update sucursal set nombre_sucursal = $1, telefono_sucursal = $2, direccion_sucursal = $3, correo_sucursal = $4 where id_sucursal = $5', [
        nombre_sucursal, telefono_sucursal, direccion_sucursal, correo_sucursal, id
    ]);
    res.status(200).json({
